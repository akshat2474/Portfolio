...
""",
  ...
  void _updateCursorPosition() {
    ...
    if (selection.baseOffset >= 0) {
      final lines = _codeController.text
    ...
    _updateState(() {
      if (_isMaximized) {
        ...
        _overlayEntry = OverlayEntry(
            builder: (context) => _buildMaximizedView(context));
    ...
    _pollingTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (statusId <= 2) {
            _updateState(() => _output =
                "Executing... Status: ${result['status']['description']}");
          } else {
            timer.cancel();
            _updateState(() {
              ...
              _executionTime =
                  double.tryParse(result['time']?.toString() ?? '0.0') ?? 0.0;
            ...
            if (_output.isEmpty && _error.isEmpty) {
              _updateState(
                  () => _output = 'Execution successful with no output.');
            }
          }
        }
      } catch (e) {
  ...
  void _showInputModal(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('Program Input (stdin)',
            style: GoogleFonts.inter(
                color: Colors.white,
                fontSize: 16,
                fontWeight: FontWeight.w600)),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            ...
            BoxShadow(
                color: _selectedLanguage.iconColor.withValues(alpha: 0.1),
                blurRadius: 60,
                spreadRadius: -5),
          ],
        ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Icon(Icons.code,
              size: _isMaximized ? 16 : 14, color: _selectedLanguage.iconColor),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              _isMaximized ? 'Live Code Studio - Full Screen' : 'Live Code Studio',
              style: GoogleFonts.inter(
                  fontSize: _isMaximized ? 13 : 11,
                  color: Colors.white70,
                  fontWeight: _isMaximized ? FontWeight.w500 : FontWeight.w400),
        ...
        children: [
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Row(
                  children: _languages
                      .map((lang) => _buildLanguageTab(lang))
                      .toList()),
            ),
          ),
          Container(
    ...
    return InkWell(
      ...
      child: Container(
        padding: EdgeInsets.symmetric(
            horizontal: _isMaximized ? 16 : 12,
            vertical: _isMaximized ? 12 : 8),
        decoration: BoxDecoration(
        ...
        child: Row(
          children: [
            Icon(language.icon,
                size: _isMaximized ? 16 : 14, color: language.iconColor),
            const SizedBox(width: 8),
            Text(language.name,
                style: GoogleFonts.firaCode(
                    fontSize: _isMaximized ? 13 : 11,
  ...
  Widget _buildRunButton() {
    return ElevatedButton.icon(
      ...
      icon: _isRunning
          ? SizedBox(
              width: _isMaximized ? 14 : 12,
              height: _isMaximized ? 14 : 12,
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor:
            _isRunning ? Colors.grey.shade700 : Colors.green.shade600,
        foregroundColor: Colors.white,
        padding: EdgeInsets.symmetric(
            horizontal: _isMaximized ? 10 : 8,
            vertical: _isMaximized ? 8 : 6),
        textStyle: GoogleFonts.inter(
            fontSize: _isMaximized ? 11 : 10, fontWeight: FontWeight.w600),
      ),
    );
  }
          ...
          child: Row(
            children: [
              Icon(Icons.code_rounded,
                  size: _isMaximized ? 16 : 14, color: Colors.grey.shade400),
              const SizedBox(width: 8),
              Text('Source Code',
                  style: GoogleFonts.inter(
                      fontSize: _isMaximized ? 13 : 11,
  ...
  Widget _buildOutputHeader() {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.terminal,
              size: _isMaximized ? 16 : 14, color: Colors.green.shade400),
          const SizedBox(width: 8),
          Text(
            'Console Output',
            style: GoogleFonts.inter(
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Icon(Icons.clear,
                    size: _isMaximized ? 14 : 12, color: Colors.white),
              ),
            ),
          ],
        ],
        ...
        children: [
          Row(children: [
            Icon(Icons.rocket_launch,
                color: Colors.blue.shade300, size: _isMaximized ? 18 : 16),
            const SizedBox(width: 8),
            Text('Live Code!',
                style: GoogleFonts.inter(
                    fontSize: _isMaximized ? 14 : 12,
                    color: Colors.blue.shade200,
                    fontWeight: FontWeight.w700)),
          ]),
          const SizedBox(height: 12),
          Text(
              '• Select a language from the tabs above.\n• Click here or start typing to go full-screen.\n• Write your code and hit "Run".',
              style: GoogleFonts.firaCode(
                  fontSize: _isMaximized ? 11 : 10,
  ...
  Widget _buildStatusBar() {
    return Container(
      ...
      child: Row(
        children: [
          Icon(_selectedLanguage.icon,
              size: _isMaximized ? 13 : 12, color: Colors.white),
          const SizedBox(width: 8),
          Text(_selectedLanguage.name,
              style: GoogleFonts.inter(
                  fontSize: _isMaximized ? 12 : 10,
                  color: Colors.white,
                  fontWeight: FontWeight.w600)),
          const SizedBox(width: 16),
          Icon(Icons.location_on,
              size: _isMaximized ? 11 : 9,
              color: Colors.white.withValues(alpha: 0.7)),
          const SizedBox(width: 4),
          Text('Ln $_cursorLine, Col $_cursorColumn',
              style: GoogleFonts.firaCode(
                  fontSize: _isMaximized ? 11 : 9,
                  color: Colors.white.withValues(alpha: 0.9))),
          const Spacer(),
          Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(3)),
            child: Text(_isRunning ? '⚡ RUNNING' : '✅ READY',
                style: GoogleFonts.inter(
                    fontSize: _isMaximized ? 10 : 9,
                    color: Colors.white,
                    fontWeight: FontWeight.w600)),
          ),
        ],
...
class _CodeEditorFieldState extends State<CodeEditorField> {
  ...
  TextSelection _previousSelectionForAutoPairing =
  ...
  void _onTextChanged() {
    ...
    if (text.length == _previousTextForAutoPairing.length &&
        selection != _previousSelectionForAutoPairing) {
      _previousTextForAutoPairing = text;
      _previousSelectionForAutoPairing = selection;
      return;
    }

    if (text.length != _previousTextForAutoPairing.length + 1 ||
    ...
    if (closingChar != null) {
      widget.controller.removeListener(_onTextChanged);
      final newText = text.substring(0, selection.base.offset) +
          closingChar +
          text.substring(selection.base.offset);
    ...
    return CodeTheme(
      ...
      child: CodeField(
        ...
        textStyle: GoogleFonts.firaCode(
            fontSize: widget.isMaximized ? 14 : 12, height: 1.5),
        expands: true,
        wrap: false,
        lineNumbers: true,
      ),
